...
"""
This file evaluate anchor's quality and give it a positive/negative tag.
Meanwhile, the script calculates anchor's vertical coordinate regression and side-refinement regression.
"""
...
def cal_iou(cy1, h1, cy2, h2):
    y_top1, y_bottom1 = cal_y_bound(cy1, h1)
    y_top2, y_bottom2 = cal_y_bound(cy2, h2)
...
# 标记anchor, 计算纵向regression和文本框边缘side-refinement loss
def tag_anchor(anchor, cnn_output, box):
    ...
    vertical_reg = []
    side_refinement_reg = []
    x_left_side = min(box[0], box[6])
    x_right_side = max(box[2], box[4])
    left_side = False
    right_side = False
